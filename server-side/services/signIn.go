...
func VendorSignin(w http.ResponseWriter, r *http.Request) {
	...
	rQuery, rArgs, rErr := statement.Select("user_id, role_id").
		From("user_roles").
		Where("user_id = ? AND role_id = ?", userId, 2).
		ToSql()
	if rErr != nil {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			SendCustomeErrorResponse(w, http.StatusNotFound, "User is not a vendor")
		} else {
